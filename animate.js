/* ============================================================
   animate.js — Mohit & Pragya Wedding Invitation
...
const SECTION_COLOURS = {
  ...
  "ev-sangeet": [
    106, 36, 16,
  ] /* warm dark auburn — matches new section bottom */,
...
const RAF = {
  ...
  add(key, fn) {
    ...
    if (!RAF._running) {
      RAF._running = true;
      RAF._id = requestAnimationFrame(RAF._tick);
    }
  },
  remove(key) {
    RAF._tasks.delete(key);
  },
};
...
function initWeddingFlowers() {
  ...
  window.addEventListener(
    "resize",
    () => {
      clearTimeout(resize._t2);
      resize._t2 = setTimeout(resize, 200);
    },
    { passive: true },
  );
...
function initNoteLilies() {
  ...
  if (window.ResizeObserver) {
    ...
    window.addEventListener(
      "resize",
      () => {
        clearTimeout(resize._t3);
        resize._t3 = setTimeout(resize, 200);
      },
      { passive: true },
    );
  }
  ...
  function drawLily(cx, baseY, stemH, swayAngle, alpha, nPetals) {
    ...
    ctx.moveTo(0, 0);
    ctx.bezierCurveTo(
      swayAngle * stemH * 0.15,
      -stemH * 0.3,
      swayAngle * stemH * 0.25,
      -stemH * 0.65,
      swayAngle * stemH * 0.18,
      -stemH,
    );
...
function initColourTransitions() {
  ...
  window.addEventListener(
    "resize",
    () => {
      clearTimeout(cacheTops._t);
      cacheTops._t = setTimeout(cacheTops, 200);
    },
    { passive: true },
  );
    ...
    requestAnimationFrame(() => {
      ...
      for (let i = 0; i < entries.length - 1; i++) {
        if (sy >= tops[i] && sy < tops[i + 1]) {
          const t = Math.max(
            0,
            Math.min(1, (sy - tops[i]) / (tops[i + 1] - tops[i])),
          );
  ...
  function onScroll() {
    ...
    requestAnimationFrame(() => {
      for (const sec of sections) {
        ...
        const shift =
          (rect.top + rect.height / 2 - window.innerHeight / 2) * rate;
